import { TimeSlider, useMediaState, VolumeSlider } from "@vidstack/react";
...
export function Volume() {
  return (
    <VolumeSlider.Root className="volume-slider group relative mx-[7.5px] inline-flex h-10 w-full max-w-[80px] cursor-pointer touch-none select-none items-center outline-none aria-hidden:hidden">
      <VolumeSlider.Track className="relative z-0 h-[5px] w-full rounded-sm bg-white/30 ring-media-focus group-data-[focus]:ring-[3px]">
        <VolumeSlider.TrackFill className="absolute h-full w-[var(--slider-fill)] rounded-sm bg-media-brand will-change-[width]" />
      </VolumeSlider.Track>
...
export function Time({ thumbnails, skips }: TimeSliderProps) {
  const totalDuration = useMediaState("duration");
  return (
    <TimeSlider.Root className="time-slider group relative mx-[7.5px] inline-flex h-10 w-full cursor-pointer touch-none select-none items-center outline-none">
      ...
      {skips?.map((skip) => {
        return (
          <div
            key={skip.skipNumber}
            className="absolute z-20 h-[5px] w-[calc(100%-2px)] rounded-[2px] bg-blue-700/50"
            style={{
              left: `${(skip.initialTime / totalDuration) * 100}%`,
              width: `${(skip.duration / totalDuration) * 100}%`,
            }}
          />
        );
      })}

      <TimeSlider.Thumb className="absolute left-[var(--slider-fill)] top-1/2 z-20 h-[15px] w-[15px] -translate-x-1/2 -translate-y-1/2 rounded-full border border-[#cacaca] bg-white opacity-0 ring-white/40 transition-opacity will-change-[left] group-data-[active]:opacity-100 group-data-[dragging]:ring-4"></TimeSlider.Thumb>
