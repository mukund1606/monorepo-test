...
import AnilistData from '../../extractors/anilistdata';
import {
  AnimeParser,
  Genres,
  IAnimeEpisode,
...
class Anilist extends AnimeParser {
  ...
  private dataExtractor = new AnilistData();
  readonly getTags = this.dataExtractor.getTags;
  ...
  advancedSearch = async (
    ...
    const availableTags = await this.dataExtractor.getTags();
  ...
  fetchRandomAnime = async (): Promise<IAnimeInfo> => {
    try {
      const ids = await this.dataExtractor.extract();
